...
async def ler_usuario(request: Request, error_message: str= None):
    return templates.Response("login.html", {"request": request, "error_menssage": error_message})
...
async def login(nome: str= Form(...), senha: str= Form(...), response: Response= None):
    ...
    if usuario:
        ...
        return RedirectResponse(url= "/login?error_message=Credenciaisinválidas", status_code=303)
...
async def ler_cadastro(request: Request):
    return templates.TemplateResponse("cadstro.html", {"request":request})
...
async def editar_usuario_form(request: Request, usuario: dict =
Depends(obter_usuario)):
 return templates.TemplateResponse("editar_usuario.html", {"request":
request, "usuario": usuario})
@app.post("/usuarios/{usuario_id}/editar")
async def editar_usuario(usuario_id: int, nome: str = Form(...), senha: str =
Form(...)):
 conexao = conectar_banco()
 cursor = conexao.cursor()
...
async def listar_agendamentos(request: Request):
 ...
 cursor.close()
 conexao.close()
 return templates.TemplateResponse("agendamentos_lista.html", {"request":
request, "agendamentos": agendamentos})
@app.get("/agendamentos/novo", response_class=HTMLResponse)
async def agendamento_form(request: Request):
 return templates.TemplateResponse("agendamento_form.html", {"request":
request})
...
async def criar_agendamento(
 ...
 if produto:
    ...
    if (produto[1] >= vonixx_extractus and
        ...
        cursor.execute("""
        UPDATE produto
        SET vonixx_extractus = %s, vonixx_bactran = %s,
        vonixx_sanitizante = %s, vonixx_sintra = %s
        WHERE id = 1
        """, (novo_extractus, novo_bactran, novo_sanitizante, novo_sintra))
        # Inserir agendamento
        sql = """
        INSERT INTO agendamento (cliente, endereco, telefone,
        data_agendamento, vonixx_extractus, vonixx_bactran, vonixx_sanitizante,
        vonixx_sintra)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
        """
        val = (cliente, endereco, telefone, data_agendamento,
...
async def agendamento_editar_form(request: Request, agendamento: dict= Depends(obter_agendamento)):
 ...
 return templates.TemplateResponse("agendamento_editar.html", {"request":
request, "agendamento": agendamento})
...
async def atualizar_agendamento(
 ...
 sql = """
 UPDATE agendamento
 SET cliente = %s, endereco = %s, telefone = %s, data_agendamento = %s,
vonixx_extractus = %s, vonixx_bactran = %s, vonixx_sanitizante = %s,
vonixx_sintra = %s
 WHERE id = %s
 """
 val = (cliente, endereco, telefone, data_agendamento, vonixx_extractus,
vonixx_bactran, vonixx_sanitizante, vonixx_sintra, agendamento_id)
 cursor.execute(sql, val)
 conexao.commit()
 cursor.close()
 conexao.close()
...
async def criar_produto(
 ...
 cursor = conexao.cursor()
 sql = """
 INSERT INTO produto (vonixx_extractus, vonixx_bactran,
vonixx_sanitizante, vonixx_sintra)
 VALUES (%s, %s, %s, %s)
 """
...
async def atualizar_produto(
 ...
 sql = """
 UPDATE produto
 SET vonixx_extractus = %s, vonixx_bactran = %s, vonixx_sanitizante = %s,
vonixx_sintra = %s
 WHERE id = %s
 """
 val = (vonixx_extractus, vonixx_bactran, vonixx_sanitizante, vonixx_sintra,
